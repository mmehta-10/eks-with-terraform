# Policy taken from https://raw.githubusercontent.com/kubernetes-sigs/aws-load-balancer-controller/main/docs/install/iam_policy.json
# Create a policy that will allow the AWS Load Balancer controller to have rights to create the ALB and register/remove target pods at the ALB

resource "aws_iam_policy" "AWS_LoadBalancer_ControllerIAMPolicy" {
  name   = "AWS_LoadBalancer_ControllerIAMPolicy"
  policy = <<POLICY
{
    "Version": "2012-10-17",
    "Statement": [
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "${replace(data.aws_eks_cluster.aws_eks.identity[0].oidc[0].issuer, "https://", "")}:sub": "system:serviceaccount:kube-system:aws-load-balancer-controller"
        }
      }
    }
  ]
...
resource "aws_iam_role_policy_attachment" "AWS_LoadBalancer_ControllerIAMPolicy" {
  policy_arn = aws_iam_policy.AWS_LoadBalancer_ControllerIAMPolicy.arn
...
resource "kubernetes_service_account" "ingress" {
  ...
  depends_on = [aws_iam_role.eks_load_balancer_controller]
}